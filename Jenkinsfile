pipeline {
  ...
  stages {
    ...
    stage ('Check-Git-Secrets') {
      ...
      sh 'curl -X "POST" "http://13.234.59.184:8080/api/v2/reimport-scan/" -H "Authorization: Token a18bbafb590275333ab0302aa1dedd57893280b3" -H "accept: application/json" -H "Content-Type: multipart/form-data" -H "X-CSRFToken: a18bbafb590275333ab0302aa1dedd57893280b3" -F "minimum_severity=Info" -F "active=true" -F "verified=true" -F "do_not_reactivate=true" -F "scan_type=Trufflehog Scan" -F "file=@trufflehog.json;type=application/json" -F "product_name=WebApp-Pipeline" -F "engagement_name=Devsecops" -F "test=1" -F "test_title=Trufflehog (Trufflehog Scan)" -F "push_to_jira=false" -F "close_old_findings=true" -F "close_old_findings_product_scope=false" -F "create_finding_groups_for_all_findings=true"'
      }
    }
    ...
    stage ('DAST') {
      steps {
        sshagent(['zap']) {
         ...
         sh 'curl -X "POST" "http://13.234.59.184:8080/api/v2/reimport-scan/" -H "Authorization: Token a18bbafb590275333ab0302aa1dedd57893280b3" -H "accept: application/json" -H "Content-Type: multipart/form-data" -H "X-CSRFToken: a18bbafb590275333ab0302aa1dedd57893280b3" -F "minimum_severity=Info" -F "active=true" -F "verified=true" -F "do_not_reactivate=true" -F "scan_type=ZAP Scan" -F "file=@zap-report.xml;type=application/json" -F "product_name=WebApp-Pipeline" -F "engagement_name=Devsecops" -F "test=4" -F "test_title=Zap Baseline Scan (ZAP Scan)" -F "push_to_jira=false" -F "close_old_findings=true" -F "close_old_findings_product_scope=false" -F "create_finding_groups_for_all_findings=true"'
        }
      }
    }
