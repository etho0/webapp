pipeline {
  ...
  stages {
    ...
    stage ('Check-Git-Secrets') {
      ...
      sh 'docker run gesellix/trufflehog --json-legacy https://github.com/etho0/webapp.git > trufflehog.json'
    ...
    stage ('DAST') {
      steps {
        sshagent(['zap']) {
         sh 'ssh -o  StrictHostKeyChecking=no ubuntu@65.2.142.213 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://65.2.172.6:8080/WebApp/ -x zap-report.xml" || true'
