pipeline {
  ...
  stages {
    ...
    stage ('DAST') {
      steps {
        sshagent(['zap']) {
         sh 'ssh -o  StrictHostKeyChecking=no ubuntu@65.2.142.213 "docker run -v $(pwd):/zap/wrk/:rw --user root -t owasp/zap2docker-stable zap-baseline.py -t http://65.2.172.6:8080/WebApp/ -o /tmp/zap-report.xml" || true'
         sh 'scp ubuntu@65.2.142.213:/tmp/zap-report.xml zap-report.xml'
