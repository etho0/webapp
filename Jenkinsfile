    ...
    stage ('DAST') {
      steps {
        sshagent(['zap']) {
         ...
         sh 'ssh -o  StrictHostKeyChecking=no ubuntu@65.2.142.213 "curl -X "POST" "http://13.234.59.184:8080/api/v2/reimport-scan/" -H "Authorization: Token 5eb4cf9d009841b5b4c7c27ce86b9f78b3dac3d0" -H "accept: application/json" -H "Content-Type: multipart/form-data" -H "X-CSRFToken: 5eb4cf9d009841b5b4c7c27ce86b9f78b3dac3d0" -F "minimum_severity=Info" -F "active=true" -F "verified=true" -F "do_not_reactivate=true" -F "scan_type=ZAP Scan" -F "file=@/var/lib/jenkins/workspace/webapp-cicd-pipeline@2/zap-report.xml;type=application/json" -F "product_name=WebApp-Pipeline" -F "engagement_name=Devsecops" -F "test=4" -F "test_title=Zap Baseline Scan (ZAP Scan)" -F "push_to_jira=false" -F "close_old_findings=true" -F "close_old_findings_product_scope=false" -F "create_finding_groups_for_all_findings=true""'
        }
      }
    }
